...
- "Non-blocker completion" means all delivery items that can be completed without pending external client inputs listed in `docs/BLOCKERS_AND_ROADMAP.md`.
...
3. Estimate quick-start by catalog room.
   - Evidence: `src/remodelator/interfaces/api/routes/estimates_base.py`
   - Evidence: `apps/web/src/panels/WorkspacePanel.tsx`
...
6. Catalog search/tree/upsert/import flows.
   - Evidence: `apps/web/src/panels/CatalogPanel.tsx`
...
10. Provider-driven billing runtime seam with fail-loud Stripe mode.
    - Evidence: `src/remodelator/application/billing_runtime.py`
...
14. Admin controls (summary/users/activity/billing/demo reset) with mutation lock safety.
    - Evidence: `tests/test_api_flow.py`
    - Evidence: `tests/test_operation_lock.py`
15. Audit retention preview/prune controls in API, CLI, and UI.
    - Evidence: `src/remodelator/interfaces/api/routes/admin.py`
...
19. API endpoint inventory generation and drift checks.
    - Evidence: `docs/API_ENDPOINTS_GENERATED.md`
20. Forgot-password explicitly deferred until email delivery exists (scope-accurate UX and docs).
    - Evidence: `apps/web/src/panels/session/AuthFormsSection.tsx`
    - Evidence: `README.md`
