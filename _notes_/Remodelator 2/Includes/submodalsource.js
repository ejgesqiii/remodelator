...
// Popup code
var gUseDivContent = false;       //Markert addition
...
var gLoading = "ModalPages/loading.html";
...
function initPopUp() {
	...
	//alert(gPopupMask + " " + gPopupContainer + " " + gPopFrame);
...
addEvent(window, "load", initPopUp);
 ...
 /**
	...
	* @argument content - url to display for IFrame, or HTML to display for inline content
	* @argument returnFunc - function to call when returning true from the window.
	* @argument useDivContent - true if content will be loaded into a DIV, false to load into an IFRAME
	* @argument title - true if content will be loaded into iFrame, false to load inline HTML content
	*/

function showPopWin(content, width, height, returnFunc, useDivContent, title) {
    //If using Div content, we need to replace the popup container's innerHTML. No need to do this if using an IFRAME because
    //the default mode is to use an IFRAME
	//alert(gPopupMask + " " + gPopupContainer + " " + gPopFrame);
    if (gPopupMask == null) {
	    //hack for IE - when calling showPopWin() using AddOnLoad, the initPopUp() function hasn't yet been called for some reason!
	    initPopUp();
	}
	//alert(gPopupMask + " " + gPopupContainer + " " + gPopFrame);
	if (useDivContent) {
	    ...
	    '<div id="popupInner">' +
		    '<div id="popupTitleBar">' +
			    ...
			    '</div>' +
		    '</div>' +
		    '<div style="width:100%;height:100%;background-color:transparent;" scrolling="auto" frameborder="0" allowtransparency="true" id="popupFrame" name="popupFrame" width="100%" height="100%"></div>' +
	    '</div>';
    }
	
	//need to re-get the popupFrame element because we just modified the gPopupContainer element!
	gPopFrame = document.getElementById("popupFrame");	
    ...
    //Markert addition - don't show fram until source has been loaded
    gPopFrame.style.display = "block";
    gUseDivContent = useDivContent;
}
	...
	for(var i = 0; i < document.forms.length; i++) {
		for(var e = 0; e < document.forms[i].length; e++){
			if(document.forms[i].elements[e].tagName == "SELECT") {
			    //DJM: This is weak; if there's a select on page with visiblity="hidden" already, then when
			    //the window closes it will get displayed, which isn't right.
